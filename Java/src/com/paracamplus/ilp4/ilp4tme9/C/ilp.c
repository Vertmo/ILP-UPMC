...
ILP_general_function ILP_find_method_global_cache (ILP_Object receiver, ILP_Method method, int argc) {
  static ILP_Class lastClass = NULL;
  static ILP_Method lastMethod = NULL;
  static int lastArgc = -1;
  static ILP_general_function lastResult = NULL;
  if(lastClass != receiver->_class || method != lastMethod || argc != lastArgc) {
    lastClass = receiver->_class;
    lastMethod = method;
    lastArgc = argc;
    lastResult = ILP_find_method(receiver, method, argc);
  }
  return lastResult;
}

ILP_general_function ILP_find_method_method_cache (ILP_Object receiver, ILP_Method method, int argc) {
  if(method->_content.asMethod.classCache != receiver->_class ||
     method->_content.asMethod.argcCache != argc) {
    method->_content.asMethod.classCache = receiver->_class;
    method->_content.asMethod.argcCache = argc;
    method->_content.asMethod.resultCache = ILP_find_method(receiver, method, argc);
  }
  return method->_content.asMethod.resultCache;
}
