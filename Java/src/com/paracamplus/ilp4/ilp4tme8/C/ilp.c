...
ILP_Object *ILP_lookup_property(ILP_Object target, ILP_Object field) {
  if(field->_class != &ILP_object_String_class) {
    ILP_domain_error("field should be a string", field);
  }
  int n = target->_class->_content.asClass.fields_count;
  ILP_Field f = target->_class->_content.asClass.last_field;
  int i;
  // On cherche d'abord dans les champs statiques
  for(i = 0; i < n; i++) {
    if(strcmp(f->_content.asField.name, field->_content.asString.asCharacter) == 0) {
      int off = f->_content.asField.offset;
      return &(target->_content.asInstance.field[off]);
    }
    f = f->_content.asField.previous_field;
  }
  // Puis dans les champs dynamiques
  ILP_Dyna_Fields df;
  for(df = target->_content.asInstance.dyna_fields; df != NULL; df = df->next) {
    if(strcmp(df->fieldname, field->_content.asString.asCharacter) == 0) {
      return &(df->value);
    }
  }
  return NULL;
...
/* Property stuff */
ILP_Object ILP_read_property(ILP_Object instance, ILP_Object fieldV) {
  ILP_Object *prop = ILP_lookup_property(instance, fieldV);
  if(prop == NULL) ILP_domain_error("Field not available in this object", instance);
  return *prop;
}

ILP_Object ILP_write_property(ILP_Object instance, ILP_Object fieldV, ILP_Object value) {
  ILP_Object *prop= ILP_lookup_property(instance, fieldV);
  if(prop == NULL) {
    ILP_Dyna_Fields fie = instance->_content.asInstance.dyna_fields;
    ILP_Dyna_Fields newFie = ILP_AllocateDynaField();
    newFie->next = fie;
    newFie->fieldname = fieldV->_content.asString.asCharacter;
    newFie->value = value;
    instance->_content.asInstance.dyna_fields = newFie;
  } else {
    *prop = value;
  }
  return ILP_FALSE;
...
ILP_Object ILP_has_property(ILP_Object instance, ILP_Object fieldV) {
  if(ILP_lookup_property(instance, fieldV) == NULL) {
    return ILP_FALSE;
  } else {
    return ILP_TRUE;
  }
