...
void ILP_coroutine_wrapper(ILP_Object (*function)(void *), struct ILP_Coroutine *co, int arity, ILP_Object *args) {
  switch(arity) {
  case 0:
    function(NULL);
    break;
  case 1: {
    ((ILP_Object (*)(void *, ILP_Object))function)(NULL, args[0]);
    break;
  }
  case 2: {
    ((ILP_Object (*)(void *, ILP_Object, ILP_Object))function)(NULL, args[0], args[1]);
    break;
  }
  case 3: {
    ((ILP_Object (*)(void *, ILP_Object, ILP_Object, ILP_Object))function)(NULL, args[0], args[1], args[2]);
    break;
  }
  case 4: {
    ((ILP_Object (*)(void *, ILP_Object, ILP_Object, ILP_Object, ILP_Object))function)(NULL, args[0], args[1], args[2], args[3]);
    break;
  }
  }
  co->_content.asCoroutine.isFinished = 1;
}

ILP_Object ILP_costart(ILP_Object closure, int arity, ...) {
  ILP_general_function f = ILP_find_invokee(closure, arity);

  ILP_Object result = ILP_AllocateCoroutine();
  result->_content.asCoroutine.isFinished = 0;
  ucontext_t *ucyield = &result->_content.asCoroutine.ucyield;
  getcontext(ucyield);
  ucyield->uc_stack.ss_sp = malloc(4096);
  ucyield->uc_stack.ss_size = 4096;
  ucyield->uc_link = &result->_content.asCoroutine.ucresume;

  ILP_Object *arguments = malloc(arity*sizeof(ILP_Object));
  va_list args;
  va_start(args, arity);
  for(int i = 0; i < arity; i++) {
    arguments[i] = va_arg(args, ILP_Object);
  }
  makecontext(ucyield, (void (*)(void))ILP_coroutine_wrapper, 4, f, result, arity, arguments);
  va_end(args);
  return result;
}

struct ILP_Coroutine *ILP_current_coroutine = NULL;

ILP_Object ILP_yield() {
  if(ILP_current_coroutine == NULL) {
    ILP_error("Pas dans une coroutine !");
  }
  struct ILP_Coroutine *co = ILP_current_coroutine;
  ILP_current_coroutine = co->_content.asCoroutine.previous;
  swapcontext(&co->_content.asCoroutine.ucyield, &co->_content.asCoroutine.ucresume);
  return ILP_FALSE;
}

ILP_Object ILP_resume(ILP_Object co) {
  if(ILP_IsA(co, Coroutine)) {
    if(!co->_content.asCoroutine.isFinished) {
      co->_content.asCoroutine.previous = ILP_current_coroutine;
      ILP_current_coroutine = (struct ILP_Coroutine *)co;
      swapcontext(&co->_content.asCoroutine.ucresume, &co->_content.asCoroutine.ucyield);
    }
  } else {
    ILP_domain_error("Not a coroutine", co);
  }
  return ILP_FALSE;
}
