...
typedef struct ILP_Object {
     ...
     union {
          ...
          struct asCoroutine {
            ucontext_t ucyield; // Contexte de l'interieur de la fonction
            ucontext_t ucresume; // Contexte de l'exterieur
            struct ILP_Coroutine *previous;
            int isFinished;
          } asCoroutine;
     }                  _content;
} *ILP_Object;
...
#define ILP_AllocateCoroutine() \
  ILP_malloc(sizeof(struct ILP_Coroutine),      \
             &ILP_object_Coroutine_class)
...
extern ILP_Object ILP_costart(ILP_Object closure, int argc, ...);
extern struct ILP_Coroutine *ILP_current_coroutine;
