...
ILP_Object ILP_sinus (ILP_Object self) {
  if (self->_class == &ILP_object_Integer_class) {
    ILP_Object result = ILP_AllocateFloat();
    result->_content.asFloat = sin(self->_content.asInteger);
    return result;
  } else if (self->_class == &ILP_object_Float_class) {
    ILP_Object result = ILP_AllocateFloat();
    result->_content.asFloat = sin(self->_content.asFloat);
    return result;
  } else {
    return ILP_domain_error("sinus : Not a number", self);
  }
}

ILP_Object ILP_makeVector (ILP_Object o1, ILP_Object o2) {
  if (o1->_class == &ILP_object_Integer_class && o2->_class == &ILP_object_Integer_class) {
    int l = o1->_content.asInteger;
    ILP_Object res = ILP_AllocateVector(l);
    res->_content.asVector.length = l;
    int i;
    for(i=0; i<l; i++) res->_content.asVector.vector[i] = o2;
    return res;
  } else {
    return ILP_domain_error("vectorMake : Not a number", o1);
  }
}

ILP_Object ILP_vectorLength (ILP_Object self) {
  if (self->_class == &ILP_object_Vector_class) {
    ILP_Object res = ILP_AllocateInteger();
    res->_content.asInteger = self->_content.asVector.length;
    return res;
  } else {
    return ILP_domain_error("vectorLength : Not a vector", self);
  }
}

ILP_Object ILP_vectorGet (ILP_Object o1, ILP_Object o2) {
  if (o1->_class == &ILP_object_Vector_class) {
    if (o2->_class == &ILP_object_Integer_class) {
      int i = o2->_content.asInteger;
      if (i>=0 && i < o1->_content.asVector.length) {
        return o1->_content.asVector.vector[i];
      } else return ILP_domain_error("vectorGet : Array index out of bound", o2);
    } else {
      return ILP_domain_error("vectorGet : Not a number", o2);
    }
  } else {
    return ILP_domain_error("vectorGet : Not a vector", o1);
  }
}
